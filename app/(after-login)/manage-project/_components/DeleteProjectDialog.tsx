...
export default function DeleteProjectDialog({
  ...
  const remove = async () => {
    setIsDeleting(true);
    const response = await HttpGateway.secureHttpDelete(`/api/projects/${project.id}`);
    setIsDeleting(false);
        ...
        <div className="space-y-4 p-6 text-center">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
              Project <span className="font-medium">{project.name}</span> akan dihapus
              permanen. Pastikan project ini sudah tidak memiliki phase atau ticket.
            </p>
          </div>
